...
请根据用户输入，按照上述脚本进行回应。如果用户确认开始，请提出第一个问题。如果用户回答了问题，请根据回答内容提出下一个问题，或者如果已经收集完三个问题的答案，请输出案例草稿。`
